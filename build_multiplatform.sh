...
help() {
  echo "Helper script to build single or multiplatform (depending on the input parameter) images."
  echo
  ...
  echo "         'gerritforge' account and images are not visible in the local registry."
  echo
}
...
DOCKER_USER=gerritforge/gerrit-ai
...
echo
echo "### Building gerrit-ai multi-platform: [$PLATFORMS] iamges"
(docker buildx build --platform "$PLATFORMS" --no-cache -t "$DOCKER_USER:${VERSION}" -t "$DOCKER_USER:$VERSION" -t "$DOCKER_USER:latest" "$DESTINATION" .)
